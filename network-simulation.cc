...
main(int argc, char* argv[])
{
    ...
    uint32_t udpPacketSizeULL = 200;
    uint32_t udpPacketSizeBe = 200;
    uint32_t lambdaULL = 300;
    uint32_t lambdaBe = 300;
    ...
    double ueVelocity = 30; // Car Speed
    ...
    cmd.AddValue("ueVelocity", "Speed of ue", ueVelocity);
    ...
    cmd.AddValue("randomStream", "Random stream value for the simulation", randomStream);
    ...
    //Read Mobility File
    std::string traceFile = "scratch/ns2mobility10.tcl";
    NS_LOG_INFO("Configuring NS2 Mobility Model using trace file: " << traceFile);
    ...
    for (uint32_t i = 0; i < ueLowLatContainer.GetN(); ++i)
    {
        ...
        Ptr<ConstantVelocityMobilityModel> mob = ue->GetObject<ConstantVelocityMobilityModel>();
        mob->SetVelocity(Vector(ueVelocity, 0.0, 0.0));  // 30 m/s Speed
    }
